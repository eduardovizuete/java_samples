...
class WebClientIntegrationTest {
    ...
    private static final ParameterizedTypeReference<Map<String, String>> MAP_RESPONSE_REF = new ParameterizedTypeReference<>() {};
    ...
    void givenDifferentWebClientCreationMethods_whenUsed_thenObtainExpectedResponse() {
        ...
        StepVerifier.create(retrieveResponse(client1.post().uri("http://localhost:" + port + "/resource")))
    ...
    void givenDifferentMethodSpecifications_whenUsed_thenObtainExpectedResponse() {
        WebClient.RequestBodyUriSpec uriSpecPost1 = createDefaultClient().method(HttpMethod.POST);
        ...
        StepVerifier.create(retrieveGetResponse(requestGet))
                .expectNextMatches(nextMap -> nextMap.get("field").equals("value"))
                .verifyComplete();
    }
    ...
    void givenDifferentBodySpecifications_whenUsed_thenObtainExpectedResponse() {
        ...
        Mono<Map<String, String>> responsePostWithNoBody = createDefaultPostResourceRequest().exchangeToMono(
                responseHandler -> {
                    ...
                    assertThat(String.valueOf(responseHandler.statusCode().value()),
                            Matchers.equalToIgnoringCase(String.valueOf(HttpStatus.BAD_REQUEST.value())));
